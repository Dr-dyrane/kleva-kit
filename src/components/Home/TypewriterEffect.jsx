...
const TypewriterEffect = ({ texts, typeSpeed = 50, eraseSpeed = 10 }) => {
	...
	useEffect(() => {
		...
		const typingEffect = () => {
			if (index < currentText.length) {
				...
				index++;
				typingRef.current = setTimeout(typingEffect, typeSpeed);
			} else {
		...
		const eraseText = () => {
			if (index > 0) {
				setDisplayedText(currentText.substring(0, index - 1));
				index--;
				typingRef.current = setTimeout(eraseText, eraseSpeed);
			} else {
	...
	}, [texts, textIndex, eraseSpeed, typeSpeed]);
